...
export function setupUiByUserStatus() {
  ...
  const loginMngEls: NodeListOf<HTMLElement> = document.querySelectorAll(
    "header > div#login-management > small"
  );
    ...
    if (user) {
      if (user.isAnonymous) {
        ...
        loginMngEls[2]
          ...
          ?.addEventListener(
            "click",
            () => (window.location.pathname = "/pets/edit.html")
          );
        // my shelters
        ...
        loginMngEls[3]
          ...
          ?.addEventListener(
            "click",
            () => (window.location.pathname = "/shelters/edit.html")
          );
        ...
        if (!allowedPages.includes(page) && !user.emailVerified) {
          ...
          alert(
            `Check your email ${user.email} for instructions to verify your account before using this operation`
          );
          ...
          const linkEls: NodeListOf<HTMLElement> =
            document.querySelectorAll(".disabled");
          for (const el of linkEls) {
            el.classList.remove("disabled");
          }
          const clearDataButton: HTMLButtonElement | HTMLElement | null =
        ...
        const signOutButton: HTMLButtonElement | null =
          loginMngEls[1].querySelector("button");
        if (signOutButton) {
          signOutButton.addEventListener("click", handleLogOut);
        }
        console.info(
          `Authenticated as 'registered with ${
...
export function setupSignInAndSignUp() {
  ...
  if (
    document.referrer.includes("/shelters/") ||
  ...
  async function handleSignInButton() {
    ...
    if (email && password) {
      try {
        const signIn: firebase.auth.UserCredential =
        ...
        const divEl: HTMLDivElement | HTMLElement | null =
          document.getElementById("error");
        if (divEl) {
          const smallEl: HTMLDivElement | HTMLElement | null =
            divEl.querySelector("small");
          if (smallEl) {
            smallEl.textContent = e.message;
          }
          divEl.hidden = false;
