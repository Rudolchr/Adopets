...
const speciesFilter: HTMLSelectElement =
  document!.querySelector("#speciesFilter")!;
fillSelectWithRange(speciesFilter, SpeciesEnum, []);
speciesFilter.addEventListener("change", () => filter());

const sexFilter: HTMLSelectElement = document!.querySelector("#sexFilter")!;
fillSelectWithRange(sexFilter, SexEnum, []);
sexFilter.addEventListener("change", () => filter());

const sizeFilter: HTMLSelectElement = document!.querySelector("#sizeFilter")!;
fillSelectWithRange(sizeFilter, SizeEnum, []);
sizeFilter.addEventListener("change", () => filter());

const ageFilter: HTMLSelectElement = document!.querySelector("#ageFilter")!;
fillSelectWithRange(
  ageFilter,
  [
    "0",
...
const compatibleWithFilter: HTMLFieldSetElement = document!.querySelector(
  "fieldset[data-bind='compatibleWithFilter']"
)!;
createChoiceWidget(
  ...
  [],
  "checkbox",
  Object.values(SpeciesEnum)
);
compatibleWithFilter.addEventListener("change", () => filter());

const suitableWithFilter: HTMLFieldSetElement = document!.querySelector(
  "fieldset[data-bind='suitableWithFilter']"
)!;
createChoiceWidget(
  suitableWithFilter,
  "suitableWithFilter",
  [],
  "checkbox",
  Object.values(SuitableWithEnum)
);
suitableWithFilter.addEventListener("change", () => filter());

const housingFilter: HTMLSelectElement =
  document!.querySelector("#housingFilter")!;
fillSelectWithRange(housingFilter, HousingEnum, []);
housingFilter.addEventListener("change", () => filter());

const shelterFilter: HTMLSelectElement =
  document!.querySelector("#shelterFilter")!;
...
function filter() {
  ...
  insertToTable((pet) => {
    ...
    const cwfAttrs = JSON.parse(
      compatibleWithFilter.getAttribute("data-value") ?? "[]"
    );
    ...
    const swAttrs = JSON.parse(
      suitableWithFilter.getAttribute("data-value") ?? "[]"
    );
    if (swAttrs.length > 0) {
